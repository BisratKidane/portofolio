...
- [ ] **ACC-02**: An admin can link a user account to a family member node, granting that user member access.
- [ ] **ACC-03**: The first-verified-user ADMIN keeps access without a linked member (explicit carve-out) and can self-link to their own member node.
- [ ] **ACC-04**: A backend guard (`requireFamilyAccess`: linked-member-or-ADMIN) enforces access on every family query and mutation server-side, not only in the frontend.
- [ ] **ACC-05**: The `User↔FamilyMember` link is added to the existing `users` table via a tracked manual `ALTER` + boot-verify step (not assumed to apply via `sync()`).
